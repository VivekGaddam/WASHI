...
// @desc    Get public feed for all users
// @route   GET /api/reports/feed
// @access  Private (logged-in users)
exports.getReportsFeed = async (req, res, next) => {
  try {
    ...
    const reports = await Report.find()
      ...
      .populate('assignedDepartment', 'name')
      .populate('notes.addedBy', 'fullName email')
      .sort({ upvotes: -1, createdAt: -1 }) // high-upvote first
      .skip(startIndex)
      .limit(parseInt(limit));
...
// @desc    Update a report by the creator
// @route   PUT /api/reports/:id
// @access  Private (user who created it)
exports.updateReportByUser = async (req, res, next) => {
  try {
    const report = await Report.findById(req.params.id);
...
exports.toggleUpvote = async (req, res, next) => {
  try {
    ...
    // Optional: adjust priority dynamically
    if (report.upvotes.length >= 10) report.priority = 'High';
    else if (report.upvotes.length >= 5) report.priority = 'Medium';
    else report.priority = 'Low';
